...
(function () {
  ...
  // Allow drop on actual-editor elements — attach directly for playwright-cli compatibility
  function _attachDragover(el) {
    if (!el._dragoverAttached) {
      el.addEventListener('dragover', e => e.preventDefault());
      el._dragoverAttached = true;
    }
  }
  function _startObserver() {
    document.querySelectorAll('.actual-editor').forEach(_attachDragover);
    new MutationObserver(mutations => {
      mutations.forEach(m => m.addedNodes.forEach(n => {
        if (n.nodeType !== 1) return;
        if (n.classList?.contains('actual-editor')) _attachDragover(n);
        n.querySelectorAll?.('.actual-editor').forEach(_attachDragover);
      }));
